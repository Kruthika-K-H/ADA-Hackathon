...
Respond ONLY with valid JSON. No text outside JSON."""
    ...
    return result
# ─────────────────────────────────────────────
# AGENT 4: ARBITRATOR AGENT (debate & consensus)
# ─────────────────────────────────────────────
...
def run_orchestration_streaming(extra_instruction: str = "", include_debate: bool = False):
    ...
    try:
        ...
        # FIX: Return structured fallback instead of empty dict
        forecast = {"area_forecasts": [], "pre_positioning": [], "insight": "Forecast unavailable"}
    # Agent 3 — Alert Agent
    yield sse("agent_start", {
        "agent": "AlertAgent",
    ...
    if include_debate:
        ...
        # Inject synthetic conflict if none detected
